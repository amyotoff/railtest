...
APP_URL = os.environ.get("APP_URL")  # URL вашего Railway-приложения, например: "https://имя-приложения.up.railway.app/"
...
def get_oil_price():
    ...
    В реальном применении можно интегрировать API с актуальными данными.
    """
...
def handle_message(message: telegram.Message):
    ...
    Если сообщение адресовано боту (начинается с "AmyBot" или является ответом на сообщение бота),
    то:
      - Если сообщение содержит "$", выводит цены биткоина и нефти.
      - Если сообщение начинается с "AmyoBot, нарисуй", генерирует картинку через OpenAI.
      - Иначе отправляет запрос в OpenAI Chat API с ролью AmyBot.
    """
    if not message.text:
        return
    ...
    is_addressed = text.lower().startswith("amybot") or (
        message.reply_to_message and 
        message.reply_to_message.from_user.username == bot.username
    )
    ...
    # Обычный режим общения через OpenAI Chat API с использованием модели "4o-mini"
    system_prompt = (
...
- **Инициализация и настройки:**  
  Код получает переменные окружения (`TELEGRAM_BOT_TOKEN`, `OPENAI_API_KEY`, `APP_URL`) и настраивает логирование, OpenAI и Telegram-бота.

- **Обработчики функций:**  
  Функции `get_bitcoin_price()` и `get_oil_price()` получают цены биткоина (с API CoinGecko) и нефти (фиксированное значение, которое можно заменить интеграцией с реальным API).

- **Основной обработчик сообщений:**  
  Функция `handle_message()` определяет, адресовано ли сообщение боту (начало с "AmyBot" или ответ на сообщение бота). Далее:
  - Если сообщение начинается с команды для генерации изображения ("AmyoBot, нарисуй"), вызывается OpenAI Image API.
  - Если сообщение содержит символ "$", бот отвечает информацией о ценах.
  - В остальных случаях запрос передается в OpenAI Chat API с системным промптом, определяющим личность AmyBot.

- **Вебхук:**  
  Flask-приложение принимает POST-запросы от Telegram по корневому URL и обрабатывает их.

Этот код можно развернуть на Railway, установив необходимые переменные окружения и настроив URL вебхука через переменную `APP_URL`.
